...
function load_results(){
    loadfile('https://raw.githubusercontent.com/germanespinosa/results/master/' + parameters['result'] + '/experiment.json', function(data) {
        ...
        for (var i = 0; i < entropies.length; i++) {
            ...
            HTML += "<div class='entropy'><div class='entropy_title'><h2>Entropy: 0." + entropies[i] + "</h2><h2>survival rate:" + Math.round(data.summary[entropyInd].survival_rate * 100) + "%</h2><h2> Avg. length:" + data.summary[entropyInd].avg_length + "</h2></div>";
